...
1. **Data Loading**: Reads protein quantification data from TMT Mosaic data output from the Hox A&B +/-GSK126 experiment. The script expects a CSV file named `Live_pq_693_TMTMosaic.csv`.

3. **Pre-Correction PCA Analysis**: Performs Principal Component Analysis on the TMT mosaic data to visualize batch effects before correction.

4. **ComBat Batch Correction**: Uses the ComBat algorithm from the `sva` package to correct for technical batch effects while preserving biological variation across:
   - HOXA +/- GSK126 treatment
   - HOXB +/- GSK126 treatment
   - Multiple biological replicates (batches 1, 2, 3)
   - Different time points

5. **Post-Correction PCA Analysis**: Re-performs PCA on batch-corrected data to validate the effectiveness of the correction.

6. **Data Export**: Outputs the batch-corrected quantification data to a CSV file for downstream analysis as well as PDF files of the PCA plots both before and after batch correction. 
...
The script expects a CSV file named `Live_pq_693_TMTMosaic.csv` containing the protein quantification data from the GSK126-treated and TMT-labeled HOXA and HOXB DNA O-MAP experiment.
...
- `Pre-ComBat_PCA_HoxAB-GSK126.pdf` - PCA plot before batch correction (auto-scaled)
